...
class FastqProcessor:
    ...
    def __init__(
        ...
        self.searcher = FuzzySearcher(similarity_threshold)
    ...
    def process_file(self):
        ...
        writer = pq.ParquetWriter(self.output_parquet, schema)

        batch_rows = []

        for seq_idx, record in enumerate(
            tqdm(SeqIO.parse(self.fastq_path, "fastq"), desc="Processing FASTQ")
        ):
            sequence = str(record.seq)

            matches = self.searcher.find_matches_in_sequence(
                sequence,
                self.query_dict,
                seq_idx
            )
